...
// https://raytracing.github.io/books/RayTracingInOneWeekend.html (11)
// .\zig-out\bin\chapter_11.exe > image.ppm
pub fn main(init: std.process.Init) !void {
    ...
    const materialLeft = material.Dielectric.init(1.5);
    const materialBubble = material.Dielectric.init(1.00 / 1.50);
    const materialRight = material.Metal.init(Color.init(0.8, 0.6, 0.2), 0);
    ...
    // create a hollow glass sphere by overlaying atop the above
    // sphere with a smaller radius.
    try world.add(
        init.gpa,
        Sphere.init(
            Point3.init(-1.0, 0.0, -1.0),
